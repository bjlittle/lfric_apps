...
module gungho_diagnostics_driver_mod
  ...
  use thickness_alg_mod,         only : thickness_alg
#endif
...
contains
  ...
  subroutine gungho_diagnostics_driver( modeldb,   &
    ...
    if (use_physics .and. use_xios_io .and. .not. modeldb%clock%is_initialisation()) then
      ...
      ! Geopotential thickness (UM STASH m01s20i002 lineage)
      call thickness_alg(modeldb%config, exner, derived_fields, theta, &
                         twod_mesh)
#endif
